...
BEGIN       -- 실행부(Execution section) 시작
    ...
    SELECT
        ...
        -- INTO 절에, 선언부의 각 변수에 일대일로, 
        -- 조회결과 저장 (*반환된 컬럼의 개수 = INTO 변수의 개수)
        -- INTO v_emp.empno, v_emp.ename, v_emp.sal
        -- INTO v_emp.empno, v_emp.ename    -- XX
    FROM emp
    WHERE empno = 7369;


    -- 실제 테이블 컬럼의 참조는, "변수명.컬렴명" 형식 사용, 개별 컬럼 참조
    -- chr(13)  : ANSI/ASCII 13 code
    -- char(10) : ANSI/ASCII 10 code
    DBMS_OUTPUT.put_line(
